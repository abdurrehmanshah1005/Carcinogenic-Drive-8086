...
.do_fuel_sound:
    ; REPLACE the old sfx_priority logic with the new call
    call play_fuel_sound_effect
    ret
.ret:
    ret
.no_hit:
...
; Shared Logic
common_game_over:
    ; 1. Draw the image (Filename is already in DX)
    call draw_320x200_image
...
update_sound_system:
    ...
    ; --- 1. Check if an SFX is active ---
    cmp word [sfx_timer], 0
    jle .play_background

    ; Play SFX Tone
    mov bx, [sfx_priority]  ; Load SFX frequency divisor
    call speaker_on_freq
    dec word [sfx_timer]    ; Decrease SFX duration
    jmp .done_sound

.play_background:
    ; --- 2. Background Music Logic ---
    inc word [bg_tick_delay]
    cmp word [bg_tick_delay], 4 ; Adjust speed of music (Higher = Slower)
    jl .keep_current_note
    
    mov word [bg_tick_delay], 0
    
    ; Advance Note
    add word [bg_music_idx], 2
    mov ax, [bg_music_len]
    shl ax, 1               ; Multiply by 2 for word offset
    cmp word [bg_music_idx], ax
    jl .play_note
    mov word [bg_music_idx], 0  ; Loop back to start

.play_note:
    mov si, bg_music_notes
    add si, [bg_music_idx]
    mov bx, [si]
...
.keep_current_note:
    ; Do nothing, let the previous note sustain
    jmp .done_sound
...
; -----------------------------------------------------------
; Play Fuel Pickup Sound (Fast Slide UP "Zhhwip")
; -----------------------------------------------------------
play_fuel_sound_effect:
    pusha
    
    mov cx, 6000        ; Start Pitch (Low / Rumble)
    
.slide_up_loop:
    mov bx, cx
    call speaker_on_freq

    ; Very short delay (Fast slide)
    push cx
    mov cx, 150         ; Delay counter (smaller = faster sound)
.d_fuel: loop .d_fuel
    pop cx

    sub cx, 60          ; Decrease divisor = Increase Pitch
    cmp cx, 1500        ; Stop when pitch gets High
    jg .slide_up_loop
...
; SOUND VARIABLES
; =============================================================
sfx_timer       dw 0        ; If > 0, play SFX. If 0, play Music
sfx_priority    dw 0        ; Current frequency for SFX

; Simple Background Tune (Divisor values: 1193180 / Frequency)
; 0 = Silence/Rest. 
bg_music_notes  dw 4560, 0, 4560, 0, 3619, 0, 3043, 0
                dw 2711, 0, 2711, 0, 3043, 0, 3619, 0
                dw 4560, 0, 0, 0, 4560, 0, 0, 0
bg_music_len    dw 22       ; Number of notes above
bg_music_idx    dw 0        ; Current note index
bg_tick_delay   dw 0        ; Tempo control
